...
func subUnits() types.Func {
	return types.Func{
		Name:        "sub_units",
		Description: "Subtracts a number of time units from a timestamp. Unit defaults to \"ms\" if not provided.",
		Args: []types.ArgSpec{
			{Name: "ts", Description: "The timestamp in milliseconds.", Type: types.NumberType, Required: true},
			{Name: "n", Description: "The number of units to subtract.", Type: types.NumberType, Required: true},
...
func subUnitsImpl(args []core.Value) (core.Value, error) {
	ts := args[0]
	if ts.Undefined {
		return core.U(), nil
	}
